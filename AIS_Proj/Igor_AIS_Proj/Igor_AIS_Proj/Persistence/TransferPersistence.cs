...
namespace Igor_AIS_Proj.Persistence
{
    public class TransferPersistence : BasePersistence<Transfer>, ITransferPersistence
    {
        ...
        public async Task<List<Transfer>> GetAllTransfersUser(int id) => await _contextEntity.AsNoTracking().Where(e => e.OriginaccountId == id).ToListAsync();
    }
}
